...
//模拟机
#define IS_SIMULA TARGET_IPHONE_SIMULATOR
//判断是否为iPad
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
    ...
    [UIView animateWithDuration:flag?0.5:0.0 animations:^{
        ...
        self.popupView.center = [self endingPoint];
        self.popupView.centerY = [self endingPoint].y + 20;
...
- (void)keyboardWillShow {
    if(!IS_IPAD) {
        [UIView animateWithDuration:0.8f animations:^{
            self.popupView.centerY = [self endingPoint].y - 80;
        } completion:^(BOOL finished) {
        }];;
    }
}
...
- (CGPoint)endingPoint {
    ...
    if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
        ...
        if (self.popupView.frame.size.width == 297) {
            ...
            if (iPhone6 || iPhone5) {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y - 30);
            }else if (iPhone4){
