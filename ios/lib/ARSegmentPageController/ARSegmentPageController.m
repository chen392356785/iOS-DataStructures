...
-(instancetype)initWithControllersForTitle:(NSArray *)titleArray2  controller:(UIViewController<ARSegmentControllerDelegate> *)controller, ...
{
    self = [super init];
    if (self) {
        NSAssert(controller != nil, @"the first controller must not be nil!");
        [self _setUp];
        titleArray =[[NSMutableArray alloc]initWithArray:titleArray2];
...
-(void)_setUp
{
    self.headerHeight =176;
    self.segmentHeight = 43;
    self.segmentToInset = 176;
...
-(void)settitle:(NSMutableArray *)arr{
    titleArray=arr;
    ...
    [self.segmentView  setTitle:titleArray];
...
-(void)_baseConfigs
{
    ...
    self.segmentView = [[ARSegmentView alloc] initWithTitleArray:titleArray];
    self.segmentView.delegate=self;
...
//-(void)segmentPageSelIndex:(NSInteger)aIndex{
//    
//}
#pragma mark选项卡 点击代理
-(void)didSegmentViewClickedButtonAtIndex:(NSInteger)aIndex{
    ...
    if (self.headerHeightConstraint.constant != self.headerHeight) {
        ...
        [scrollView setContentOffset:CGPointMake(0,-(self.segmentHeight + self.headerHeight))];
    }
    //add obsever
