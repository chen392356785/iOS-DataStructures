...
static const char kKeyChainUDIDAccessGroup[] = "83KC7TE42W.com.infohold";


@implementation IHUDIDTools
...
+ (NSString*)UDID
{
    ...
    NSString *udid = [IHUDIDTools getUDIDFromKeyChain];
    ...
    if (udid == nil) {
        ...
        if (oldUUID != nil) {
            udid = [[NSString alloc] initWithString:oldUUID];
        }
        else
        {
            if (version >= 6.0)
...
//                NSMakeCollectable(theUUID);
                udid = [[NSString alloc] initWithString:(__bridge NSString*)string];
        ...
        [IHUDIDTools settUDIDToKeyChain:udid];
...
//+ (NSString*)_UDID_iOS6
//{
//    return [IHUDIDTools getMacAddress];
//}
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    NSString *udid      = nil;
    queryErr = SecItemCopyMatching((CFDictionaryRef)dictForQuery,(__bridge void *)udidValue);
    
    NSMutableDictionary *dict = nil;
    [dictForQuery setValue:(id)kCFBooleanTrue forKey:(id)kSecReturnAttributes];
    queryErr = SecItemCopyMatching((CFDictionaryRef)dictForQuery,(__bridge void *)dict);
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([IHUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [IHUDIDTools updateUDIDInKeyChain:udid];
        return YES;
    }
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    SecItemCopyMatching((CFDictionaryRef)dictForQuery, (__bridge void*)queryResult);
    if (queryResult) {
