...
- (void)initNavBtn
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"chat_back@2x"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(navLeftBtn_Click)];
//    UIImage *shareImg=Image(@"navBackBtn.png");
//    UIButton* backBut=[UIButton buttonWithType:UIButtonTypeCustom];
//    backBut.frame=CGRectMake(0, 0, 40, 40);
//    [backBut addTarget:self action:@selector(navLeftBtn_Click) forControlEvents:UIControlEventTouchUpInside];
//    [backBut setImage:shareImg forState:UIControlStateNormal];
//    UIBarButtonItem *backButItem=[[UIBarButtonItem alloc]initWithCustomView:backBut];
//    self.navigationItem.leftBarButtonItem = backButItem;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[ZLPhotoTool sharePhotoTool] requestImageForAsset:asset size:size resizeMode:PHImageRequestOptionsResizeModeExact completion:^(UIImage *image) {
		...
		for (ZLSelectPhotoModel *model in self->_arraySelectPhotos) {
            if ([model.localIdentifier isEqualToString:asset.localIdentifier]) {
