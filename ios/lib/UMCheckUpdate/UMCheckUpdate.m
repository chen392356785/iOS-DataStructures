...
- (void)checkUpdateWithAppID:(NSString *)appId
    ...
    if (!appId || [appId length] == 0) {
        NSLog(@"UMCheckUpdateLOG: please provide appId!");
        return;
    }
...
- (BOOL)checkAppUpdate:(NSString *)serverUrl {
    ...
    if (jsonResponseString != nil) {
        ...
        if (error != nil) {
            NSLog(@"(Error    checkAppUpdate) %@", error);
        }
        return NO;
    }
}
...
- (void)executeUpdate:(NSDictionary *)itunesResponse {
    ...
    NSLog(@"通过appStore获取的版本号是:%@, 当前版本号:%@",newVersion, self.appVersion);
