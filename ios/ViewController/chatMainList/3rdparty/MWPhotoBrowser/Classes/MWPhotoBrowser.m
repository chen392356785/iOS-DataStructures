...
#pragma clang diagnostic ignored "-Wmismatched-return-types"
	_toolbar = [[UIToolbar alloc] initWithFrame:[self frameForToolbarAtOrientation:(NSInteger)self.interfaceOrientation]];
...
- (void)savePhoto {
    ...
    if (author == kCLAuthorizationStatusRestricted || author ==kCLAuthorizationStatusDenied) {
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            NSLog(@"====");
			id <MWPhoto> photo = [self photoAtIndex:self->_currentPageIndex];
            if ([photo underlyingImage]) {
...
//                PHAssetChangeRequest *req = [PHAssetChangeRequest creationRequestForAssetFromImage:[photo underlyingImage]];
            ...
            if (success == YES) {
                NSLog(@"-----");
                [IHUtility addSucessView:@"保存成功！" type:1];
            }else{
