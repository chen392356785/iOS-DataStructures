...
- (void)viewDidLoad
{
    ...
    [self setTitle:self.nickName];
    
    if ([self isChatGroup]) {
        [self setRightButtonStyle:IHNavButtonStyleChatInfo];
    }
...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0, self.view.frame.size.width, SCREEN_HEIGHT - [DXMessageToolBar defaultHeight]) style:UITableViewStylePlain];
...
{
    if (_chatToolBar == nil) {
        _chatToolBar = [[DXMessageToolBar alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.tableView.frame), self.view.frame.size.width, [DXMessageToolBar defaultHeight])];
        ...
        _chatToolBar.moreView.backgroundColor = RGBACOLOR(237, 237, 237, 1);
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
    ...
    if ([eventName isEqualToString:kRouterEventTextURLTapEventName]) {
        ...
        [self addWaitingView];
      [network getUserInfoByHxName:model.username success:^(NSDictionary *obj) {
          
         
          UserChildrenInfo *mod=[[UserChildrenInfo alloc]initWithDictionary:obj[@"content"] error:nil];
          
          [network selectHxUserType:model.username success:^(NSDictionary *obj) {
              
              if ([obj[@"content"][@"user_type"] intValue]==1) {
              ...
              }else if ([obj[@"content"][@"user_type"] intValue]==2){
               
                  [network selectFlowCarUserByMobile:[obj[@"content"][@"user_id"] intValue] success:^(NSDictionary *obj) {
                      [self removeWaitingView];
...
{
    if (_conversationType==eConversationTypeGroupChat) {  //群聊显示头像
...
{
    NSDictionary *ext = [self getExtDic];
    if (_isRobot) {
...
- (void)didChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.origin.y = 0;
        rect.size.height = WindowHeight - toHeight;
...
-(NSDictionary *)getExtDic{
    ...
    NSDictionary* ext = [NSDictionary dictionaryWithObjectsAndKeys:
                         [NSString stringWithFormat:@"%@",USERMODEL.hxUserName],kFromeUserID,
                         USERMODEL.nickName,kFromeNickName,
                         USERMODEL.userHeadImge80,kFromeHeadImage,
                         [NSString stringWithFormat:@"%@",_chatter],kToUserID,
                         self.nickName,kToNickName,
                         self.HeadimgUrl,kToHeadImage,
               //          self.toUserID,kToUser_ID,
                         nil];
    ...
    [dic setObject:ext2 forKey:[NSString stringWithFormat:@"%@",_chatter]];
    [IHUtility setUserDefaultDic:dic key:kMessageDic];
