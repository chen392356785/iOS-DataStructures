...
@interface YLWebViewController () <WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate>
...
{
    if (!_progressView){
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 1, iPhoneWidth, 2)];
...
-(void)viewDidLoad {
    ...
    [self setRightButtonStyle:IHNavButtonStyleShare];
...
- (void)home:(id)sender
{
    if (self.i==1) {
        ...
        [self ShareUrl:self withTittle:self.NameTitle content:self.content withUrl:[NSString stringWithFormat:@"%@&platform=wechat",self.mUrl] imgUrl:@""];
    }else
    {
        [self ShareUrl:self withTittle:self.NameTitle content:@"" withUrl:[NSString stringWithFormat:@"%@",self.mUrl] imgUrl:@""];
    }
}
