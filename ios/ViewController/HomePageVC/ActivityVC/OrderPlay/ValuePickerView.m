...
#define HEIGHT_PICKER HEIGHT_OF_POPBOX + 100 + self.dataSource.count * 20
...
- (instancetype)init
{
    if (self = [super initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)]) {
        ...
        self.pickerHeight = HEIGHT_OF_POPBOX + 100;
        //初始化子视图
        [self initSubViews];
    }
    return self;
...
{
    //底层视图
    self.bottomView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, self.pickerHeight);
    
    //选择器
    self.statePicker.frame = CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, self.pickerHeight - 40);
    
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 40);
    
    //完成按钮
    self.finishBtn.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 70, 5, 70, 30);
    
    //取消按钮
    self.cancelBtn.frame = CGRectMake(0, 5, 70, 30);
    
    //标题
    self.titleLabel.frame = CGRectMake(70, 5, [UIScreen mainScreen].bounds.size.width - 140, 30);
}
...
- (void)removeSelfFromSupView
{
    ...
    if (selfFrame.origin.y == [UIScreen mainScreen].bounds.size.height - self.pickerHeight) {
