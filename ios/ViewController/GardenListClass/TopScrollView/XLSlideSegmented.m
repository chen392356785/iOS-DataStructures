...
//    if (_titles.count <= 3) {
//        return CGSizeMake(iPhoneWidth/_titles.count, _collectionView.bounds.size.height);
//    }
//    return CGSizeMake(iPhoneWidth / 3, _collectionView.bounds.size.height);
...
//    item.backgroundColor = indexPath.row == _selectedIndex ? [UIColor whiteColor] : [UIColor clearColor];
    item.textLabel.font = [UIFont systemFontOfSize:font(15)];
    item.lineLabel.backgroundColor = indexPath.row == _selectedIndex ? _itemSelectedColor : [UIColor clearColor];
    return item;
}
