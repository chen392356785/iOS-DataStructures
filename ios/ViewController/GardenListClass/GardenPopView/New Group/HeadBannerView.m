...
+(instancetype)initWithFrame:(CGRect)frame
    ...
    HeadBannerView *scrollView = [[self alloc] initWithFrame:frame];
...
-(void)createPageControl{
    ...
    _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake((self.width - (self.data.count - 1)*15)/2.0, ZXMainScrollViewHeight + kWidth(10) , (self.data.count - 1)*15, kWidth(10))];
//    [_pageControl setValue:[[UIImage imageNamed:@"selected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forKeyPath:@"_currentPageImage"];
//    [_pageControl setValue:[[UIImage imageNamed:@"normal"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forKeyPath:@"_pageImage"];
    _pageControl.pageIndicatorTintColor = kColor(@"#C8C8C8");
    _pageControl.currentPageIndicatorTintColor = kColor(@"#05C1B0");
...
- (void)reloadImage {
    ...
    if ((int)contentOffset.x == (int)ZXMainScrollViewWidth) {
        return;
    }
