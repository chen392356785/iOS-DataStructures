...
static CGFloat sendBtnW = 24.0f;
static CGFloat sendBtnH = 24.0f;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundColor = kColor(@"#DCDCDC");
        
        UITextView *textView = [[UITextView alloc] init];
        textView.delegate    = self;
        textView.textColor   = kColor(@"#030303");
        textView.backgroundColor = kColor(@"#FFFFFF");
        textView.font = textViewFont;
        textView.layer.cornerRadius  = 2;
        textView.layer.masksToBounds = YES;
        textView.layer.borderWidth   = 0.5;
        textView.layer.borderColor   = [kColor(@"#DCDCDC") CGColor];
        textView.layoutManager.allowsNonContiguousLayout = NO;
        [self addSubview:textView];
        self.textView = textView;
        UIButton *sendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//        [sendBtn setTitle:@"发送" forState:UIControlStateNormal];
        [sendBtn setImage:[kImage(@"send_img") imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    [UIView animateWithDuration:0.15 animations:^{
        [self setFrame:CGRectMake(self.frame.origin.x, Main_Screen_Height-(self->_keyBoardHight+superHeight), self.frame.size.width, superHeight)];
    }];
