...
- (instancetype)initWithTitle:(NSString *)title attributeMessage:(NSMutableAttributedString *)attrMessage delegate:(id<YQAlertViewDelegate>)delegate buttonTitles:(NSString *)buttonTitles, ...{
    ...
    [yqAlert.attrMessage addAttribute:NSFontAttributeName value:KFont(MESSAGE_FONT_SIZE) range:NSMakeRange(0, attrMessage.length)];
...
//    NSString *text = message;
    ...
    yqAlert.message = [message stringByReplacingOccurrencesOfString:@"\\n" withString:@"\n"];
    yqAlert.buttonTitleArray = buttonTitleArray;
    yqAlert.delegate = delegate;
    [yqAlert showBackground];
...
- (void)SetupSubviews
{
    ...
    titleLabel.font = KFont(TITLE_FONT_SIZE);
    titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
    [self.contentView addSubview:titleLabel];
    self.titleLabel = titleLabel;
    
    UILabel *messageLabel = [[UILabel alloc] init];
    messageLabel.textColor = THTextColor;
    messageLabel.textAlignment = NSTextAlignmentCenter;
    messageLabel.numberOfLines = 0;
    messageLabel.font = KFont(MESSAGE_FONT_SIZE);
    [self.contentView addSubview:messageLabel];
    self.messageLabel = messageLabel;
    
    UIView *horLine = [[UIView alloc] init];
    horLine.backgroundColor = THBaseLightGray;
...
- (void)layoutSubviews{
    ...
    {
        self.titleLabel.frame = CGRectMake(0, 10, contentViewWidth, TITLE_HEIGHT);
    }
    ...
    messageAttrs[NSFontAttributeName] = KFont(MESSAGE_FONT_SIZE);
    ...
    if (self.buttonTitleArray.count > 0)
    {
        ...
        self.horLine.frame = CGRectMake(0, CGRectGetMaxY(self.messageLabel.frame) + 20, contentViewWidth, lineSize);
        
        CGFloat buttonW = contentViewWidth / _buttonTitleArray.count;
        CGFloat buttonH = 38.0f;
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(index * (buttonW+lineSize), CGRectGetMaxY(self.horLine.frame), buttonW, 38)];
            button.titleLabel.font = KFont(BUTTON_FONT_SIZE);
            [button setTitle:buttonTitle forState:UIControlStateNormal];
            [button setTitleColor:THTextColor forState:UIControlStateNormal];
            ...
            if (index < _buttonTitleArray.count - 1)
            {
                ...
                verLine.backgroundColor = THBaseLightGray;
            ...
            if (index == _buttonTitleArray.count - 1)
            {
                button.backgroundColor = THBaseColor;
