...
- (void)buildFooterView
{
    ...
    self.skipButton.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:16];
    self.skipButton.frame=CGRectMake(iPhoneWidth/2-40 , self.pageControl.frame.origin.y-15, 80 , self.pageControl.frame.size.height);
    [self.skipButton setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
    [self.skipButton setTitleColor:THTextColor forState:UIControlStateNormal];
...
- (void)showPanelAtPageControl {
    ...
    [self.scrollView setContentOffset:CGPointMake(self.currentPageIndex * iPhoneWidth, 0) animated:YES];
}
