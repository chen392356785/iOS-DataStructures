...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    for (UICollectionViewLayoutAttributes *attr in array) {
        if(ABS(minDelta) > ABS(attr.center.x - centerX)) {
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:DiscernSuccessListScrollIdentifier object:attr.indexPath];
        }
    }
...
//        if (delay > kWidth / 2) {
//            scale = 1 - (kWidth / 2 / self.collectionView.frame.size.width);
//        }
//
//        // 设置缩放比例
//        attr.transform3D = CATransform3DMakeScale(scale, scale, scale);
        ...
        CGFloat scale = fabs(cos(apartScale * M_PI / 2.8));
