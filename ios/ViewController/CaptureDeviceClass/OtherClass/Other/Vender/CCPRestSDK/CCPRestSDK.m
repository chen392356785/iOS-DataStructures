...
//    NSMutableDictionary* ret = [self goRestUrlWithType:@"POST" andUrlStr:requestUrl andBodyData:[xmlBody dataUsingEncoding:NSUTF8StringEncoding] andTimestamp:[self getMainAuthorization:timestamp]andRequestType:ERequestType_SendSMS];//注意最后一个参数是主帐号鉴权
	NSMutableDictionary* ret = [self goRestUrlWithType:@"POST" andUrlStr:requestUrl andBodyData:[xmlBody dataUsingEncoding:NSUTF8StringEncoding] andTimestamp:[self getMainAuthorization:timestamp]andRequestType:0];//注意最后一个参数是主帐号鉴权
...
//解析返回数据
//-(NSMutableDictionary*)builderWithBody:(NSData*)responseData andRequestType:(NSInteger)requestType
//{
...
//发起rest请求
-(NSMutableDictionary*)goRestUrlWithType:(NSString*)type andUrlStr:(NSString*) urlStr andBodyData:(NSData*) bodyData andTimestamp:(NSString*)timestamp andRequestType:(NSInteger) requestType
{
