...
- (void)viewDidLoad {
 ...
 //    [self.navigationController.navigationBar setShadowImage:[UIImage new]];
   ...
   // [self.navigationController.navigationBar lt_setBackgroundColor:RGBA(249, 249, 249, 1)];
    ...
    [geocoder reverseGeocodeLocation:newLocation
                   completionHandler:^(NSArray *placemarks, NSError *error){
                       ...
                       for (CLPlacemark *place in placemarks) {
                           ...
                           self.showUserLocationBlock(_province,_city,newLocation.coordinate.latitude,newLocation.coordinate.longitude);
                           isLocation=YES;
                         // NSLog(@"%@",_userLocation);
                                        // 位置名
...
//    __weak ShareView *weakShare=shareView;
    ...
    shareView.selectShareMenu=^(NSInteger index){
        ...
        if (index==6) {
          ...
          [IHUtility SharePingTai:title url:sharUrl imgUrl:imgUrl content:content PlatformType:index controller:vc];
...
//    shareView.selectBtnBlock=^(NSInteger index){
        ...
        if (index==SelectWXhaoyouBtnBlock) {
           ...
           NSString *str=[NSString stringWithFormat:@"%@，加入%@，携手共赢",KAppTitle,KAppName];
             [IHUtility SharePingTai:str url:[NSString stringWithFormat:@"%@register/register.html?id=%@&appkey=%@",shareURL,[IHUtility base64WithString:USERMODEL.userID],APP_KEY] imgUrl:@"" content:[NSString stringWithFormat:@"用%@找苗木、找人脉、找企业，即刻邀请好友加入，直接送你微信红包",KAppName] PlatformType:1 controller:self];
            
            
        }else if (index==SelectWXpengyouBtnBlock){
            NSString *str=[NSString stringWithFormat:@"%@，加入%@，携手共赢",KAppTitle,KAppName];
            [IHUtility SharePingTai:str url:[NSString stringWithFormat:@"%@register/register.html?id=%@&appkey=%@",shareURL,[IHUtility base64WithString:USERMODEL.userID],APP_KEY] imgUrl:@"" content:[NSString stringWithFormat:@"用%@找苗木、找人脉、找企业，即刻邀请好友加入，直接送你微信红包",KAppName] PlatformType:2 controller:self];
...
-(void)ShareUrl:(UIViewController *)vc withTittle:(NSString *)tittle content:(NSString *)content withUrl:(NSString *)urlStr imgUrl:(NSString *)imgURL
{
        ...
        shareView.selectShareMenu=^(NSInteger index){
            ...
            [IHUtility SharePingTai:tittle url:urlStr imgUrl:imgURL content:str PlatformType:index controller:vc];
...
-(void)taskShareApp:(UIViewController *)vc{
    ...
    shareView.selectBtnBlock=^(NSInteger index){
        ...
        if (index==SelectWXhaoyouBtnBlock) {
            ...
            [IHUtility SharePingTai:str url:@"http://www.miaoto.net/" imgUrl:@"" content:[NSString stringWithFormat:@"我一直在用%@找苗木、搜人脉、找企业。邀你一起来体验，积分还可抵现金用哦",KAppName] PlatformType:1 controller:self];
            
          
        }else if (index==SelectWXpengyouBtnBlock){
            NSString *str=[NSString stringWithFormat:@"HI，朋友，友谊的小船不会翻，送你100个%@积分，抵现金用",KAppName];
            [IHUtility SharePingTai:str url:@"http://www.miaoto.net/" imgUrl:@"" content:[NSString stringWithFormat:@"我一直在用%@找苗木、搜人脉、找企业。邀你一起来体验，积分还可抵现金用哦",KAppName] PlatformType:2 controller:self];
        }
...
-(void)Shareinformation:(NSString *)userId name:(NSString *)name phone:(NSString *)phone adress:(NSString *)adress imgUrl:(NSString *)imgUrl vc:(UIViewController *)vc{
    ...
    shareView.selectShareMenu=^(NSInteger index){
        ...
        [IHUtility SharePingTai:str url:adress imgUrl:imgUrl content:content PlatformType:index controller:self];
...
-(void)showLoginViewWithType:(LaginType)type
{
    ...
    loginView.selectSurnBlock=^(NSString *str,NSString *str2){
        [self addWaitingView];
    ...
    loginView.selectYZMBlock=^(NSString *phone){
        
        [network getSendRegisterSms:phone type:2 chanle:channel  success:^(NSDictionary *obj) {
            channel++;
            ...
            [ConfigManager countdownSecond:60  returnTitle:^(NSString *title)
             {
                 ...
                 if (ConfigManager.seconds>0) {
                     [loginView.loginBtn setTitle:title forState:UIControlStateNormal];
...
- (void)viewDidLoad
{
  ...
  //  [self.navigationController.navigationBar lt_setBackgroundColor:RGBA(249, 249, 249, 1)];
    ...
    UIImage *bgImg=Image(@"tabBartitleBg.png");
    bgImg=[bgImg stretchableImageWithLeftCapWidth:bgImg.size.width/2 topCapHeight:bgImg.size.height];
    //        UIImage *bgImg = [self imageWithColor:RGBA(255,225, 82, 1) andSize:CGSizeMake(_deviceSize.width, 64)];
