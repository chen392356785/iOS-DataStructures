...
- (id)initWithDict:(NSDictionary *)resultDict
{
	...
	if (self = [super init]) {
		...
		self.statusCode = 0;
		self.statusMessage = @"支付异常，请重试！";
		
        do {
 
            self.statusCode = [[resultDict objectForKey:@"resultStatus"] intValue];
            self.statusMessage = [resultDict objectForKey:@"memo"];
...
- (NSString *)description {
	...
	[description appendFormat:@"\tstatusCode=%zd\n", self.statusCode];
