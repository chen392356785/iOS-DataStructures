...
@implementation NSAttributedString (YVText)
...
+ (NSMutableAttributedString *)ls_changeCorlorsWithArray:(NSArray <UIColor *> *)colors
	...
	return [NSAttributedString ls_changeCorlorsWithArray:colors fonts:nil TotalString:totalStr SubStringArray:subArray];
}
...
+ (NSMutableAttributedString *)ls_changeCorlorsWithArray:(NSArray <UIColor *> *)colors
	...
	for (NSInteger i = 0; i<subArray.count; i++) {
		NSString *subString = [subArray objectAtIndex:i];
		UIColor *subColor = [colors objectAtIndex:i];
		UIFont *font = fonts ? [fonts objectAtIndex:i] : nil;
		NSMutableArray *array = [self ls_getRangeWithTotalString:totalStr SubString:subString];
		for (NSNumber *rangeNum in array) {
			NSRange range = [rangeNum rangeValue];
			[attributedStr addAttribute:NSForegroundColorAttributeName value:subColor range:range];
			!font ?: [attributedStr addAttribute:NSFontAttributeName value:font range:range];
		}
	}
	return attributedStr;
}
...
+ (NSMutableArray *)ls_getRangeWithTotalString:(NSString *)totalString SubString:(NSString *)subString {
    ...
    if (subString == nil && [subString isEqualToString:@""]) { return nil; }
...
/**
 拼接图片到富文本=
 */
+ (NSAttributedString *) yv_addImage:(UIImage *)image bounds:(CGRect)bounds {
 ...
 *  @return 生成的富文本
 */
+ (NSMutableAttributedString *)ls_changeCorlorWithColor:(UIColor *)color TotalString:(NSString *)totalStr lineSpacing:(CGFloat)lineSpacing  SubStringArray:(NSArray *)subArray {
...
@implementation NSMutableAttributedString (YVAdd)
...
- (void) setLinSpacing:(CGFloat)lineSpacing {
	...
	[self enumerateAttributesInRange:NSMakeRange(0,self.length) options:NSAttributedStringEnumerationReverse usingBlock:^(NSDictionary<NSAttributedStringKey,id> * _Nonnull attrs, NSRange range, BOOL * _Nonnull stop) {
		__strong typeof(weakSelf)strongSelf = weakSelf;
		[strongSelf addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:range];
	}];
